...
def _axis_to_rotation(axis, target_axis=coordinates.Y):
    """
    Align input-axis to axis of jointShape
    """
    cds = coordinates()
...
def _func_links(parent_coords, child_coords, scale=1.0, color=None, **kwargs): ## BOX type
    ...
    geometry = {'primitive': 'box',
                'args': {'x': 0.12*scale, 'y': len_, 'z': 0.12*scale, 'color': [0, 1, 0] },
                }
    return (cds, geometry)
...
class RobotTree(nx.DiGraph):
    ...
    def make_list(self, start_node=None, use_params=True, add_coords=True):
        """
        ...
        if use_params:
            ...
            if add_coords and 'coords' in self.nodes[start_node]:
                cds = self.nodes[start_node]['coords']
                res = ru.make_coords_map(cds, method='rotation')
                st_params['translation'] = res['translation']
                st_params['rotation'] = res['rotation']
    ...
    def merge_link_to_parent(self, name):
        #### child translation if relative //
        #### move child-geometries to parent
        #### merge mass-parameter
        pass
    ...
    def _parse_body(self, in_body):
        ...
        for l in in_body.links:
            ...
            params['type'] = 'link'
            masspara = {}
            masspara['mass'] = l.mass
            masspara['COM'] = l.c.tolist()
            masspara['inertia'] = l.I.tolist()
            params['mass-param'] = masspara
            if l is rt:
...
def makeShape(module, primitive=None, args=None, coords=None):
    ...
    if primitive == 'box':
        ...
        _gfunc = module.makeCapsule
    elif primitive == 'torus':
        _gfunc = module.makeTorus
    elif primitive == 'tetrahedron':
        _gfunc = module.makeTetrahedron
    elif primitive == 'extrusion':
        _gfunc = module.makeExtrusion
    elif primitive == 'scen':
        _gfunc = module.loadScene
    elif primitive == 'mesh':
        _gfunc = module.loadMesh
    elif primitive == 'scene_graph':
        ## not implemented yet
        if args is not None:
            pass ## add args['SgNode'] = xxx
    else:
        print('makeShape, Failed')
        return
    ###
    _res = _gfunc(**args)
    if coords is not None:
        _res.newcoords(coords)
    return _res
...
#        ## build geometry
#        self._buildGeomForLink(rbt, root_node)
#        node_map = rbt.nodes[link_node]
#        cds = node_map['coords'] if 'coords' in node_map else coordinates()
#        params = node_map['params']
#        jp = params['joint'] if 'joint' in params else {}
#        jtype = jp['type'] if 'type' in jp else 'free'
#        ## override mass-parameter
#        massParam=None
#        if 'mass-param' in params:
#            massParam=params['mass-param']
#            ## convert mass param on cds to on j
#            ## !HOTFIX!
#            if 'inertia' in massParam:
#                class _tmp:
#                    pass
#                _p = _tmp()
#                _p.T = cds.cnoidPosition
#                _p.mass = massParam['mass']
#                _p.c = massParam['COM']
#                _p.I = massParam['inertia']
#                newmass, new_c, new_I = ru._mergeMassProperty(j, 0, fv(0., 0., 0.), fv([0., 0., 0.], [0., 0., 0.], [0., 0., 0.]), _p)
#                massParam['COM'] = new_c
#                massParam['inertia'] = new_I
...
#                                        **args)
    ...
    def _addRoot(self, rbt, root_node):
        return self._addJoint(rbt, None, root_node, root=True)

    def _addJoint(self, rbt, parent_lk, link_node, root=False):
        ## build geometry
        ...
        _build_jtype = Link.JointType.FreeJoint
        if jtype == 'revolute' or jtype == 'revolute_yaw':
            _build_jtype = Link.JointType.RevoluteJoint
        elif jtype == 'linear':
            _build_jtype = Link.JointType.PrismaticJoint
        elif jtype == 'fixed':
            _build_jtype = Link.JointType.FixedJoint
        elif jtype == 'ball':
            _build_jtype = RobotBuilder.JointType.Ball
        elif jtype == 'free' or jtype == 'root':
            pass
        #
        j  = self.createJointShape(jointType=_build_jtype)
        ##
        j.newcoords(cds)
        if 'axis' in jp:
            axis = jp['axis']
        ...
        if 'mass-param' in params:
            ...
            if 'inertia' in massParam:
                ...
                massParam['inertia'] = new_I
        if root:
            return self.createLinkFromShape(name=link_node,
                                            root=True,
                                            overwriteMassparam=massParam,
                                            **args)
